...
TableKit.prototype = {
	initialize : function(elm, options) {
		...
		this.id = table.id;
		var op = TableKit.option('sortable resizable editable linkable', this.id);
		if(op.sortable) {
...
// Here's the one I added - converts tables containing the class linkable to links of the first row. 
// Re-initializes on re-sort so as not to screw up the alternating rows
// NOTE: If row has 2 or more links, will link on the first found. To disable on multi-link table rows, see comment below
function ConvertRowsToLinks(xTableId){
	var doMe = false;
	var checkme = document.getElementById(xTableId).className.split(" ");
    for(i=0;i<checkme.length;i++){
    	if (checkme[i] == "linkable"){
    		doMe = true;
    	}
	}
	if (doMe == true ){
		...
		for(i=0;i<rows.length;i++){
			...
			var temp = rows[i].className; 
			// Uncomment the line below and comment out the following if you want to disable row linking when there is more than one 
			// if ((link[0] !== undefined) && (link.length == 1)) {
			if (link[0] !== undefined) {
				rows[i].onclick = new Function("document.location.href='" + link[0].href + "'");
			    rows[i].onmouseover = new Function("this.className='" + rows[i].className + " highlight'");
			    rows[i].onmouseout = new Function("this.className='" + rows[i].className.gsub(" highlight","") + "'");
			}
...
Object.extend(TableKit, {
	...
	reloadTable : function(table){
	  ...
	  var op = TableKit.option('sortable resizable editable linkable', table.id);
	  ...
	  if(op.linkable) {ConvertRowsToLinks(table);}
	},
	reload : function() {
